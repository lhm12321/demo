...
<form id="pagerForm" method="post" action="rkd" >
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="rkd" method="post">
		<div>
		...
		<tbody>
			...
			<c:forEach var="pojo" items="${listPage.list}" varStatus="status" >
				<tr target="pojo_id" rel="${pojo.id}" align="center">
					...
					<td><a target="dialog" width="500" height="400" href="rkd/rkd_xq?id=${pojo.id}" rel="rkd_xq" title="详细" ><span>${pojo.name}</span></a></td>
					<td>${pojo.sum}</td>
					<td>${pojo.total}</td>
